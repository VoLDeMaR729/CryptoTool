...
// Оборачиваем весь код в extern "C" для корректной сборки разделяемой библиотеки
extern "C" {

// --- Внутренние функции TEA ---

// Подготовка 128-битного ключа из строкового пароля
...
// Шифрование одного блока (64 бит) алгоритмом TEA
...
// Дешифрование одного блока (64 бит) алгоритмом TEA
...
// Проверка валидности числового ключа для перестановок
bool isValidKey(const char* key, int length) {
    ...
    for (int i = 0; i < length; ++i) {
        ...
        int digit = key[i] - '0';
        // Проверяем выход за границы и дубликаты
...
// 1. Реализация Атбаш (in-place замена)
...
char* doubleTransposition(const char* data, long size, const char* colKey, const char* rowKey, bool encrypt, long& newSize) {
    ...
    // Создание таблиц перестановки (Динамическое выделение памяти по стандарту)
    ...
    if (!colPerm || !rowPerm || !colInvPerm || !rowInvPerm) {
        free(paddedData); free(colPerm); free(rowPerm); free(colInvPerm); free(rowInvPerm);
        return NULL;
    }

    // Инициализация массивов перестановки
    for (int i = 0; i < C; ++i) {
...
// 3. Реализация TEA с поддержкой PKCS#7 Padding
char* teaProcess(const char* data, long size, const char* password, bool encrypt, long& newSize) {
    ...
    if (encrypt) {
        ...
        memset(buffer + size, (char)padding_needed, padding_needed); // Применяем PKCS#7
    ...
    // Дешифрование
        ...
        if (padVal > 0 && padVal <= block_size) {
            ...
            newSize = paddedSize; // Ошибка паддинга, оставляем оригинальный размер
        }
    }
    return buffer;
}
